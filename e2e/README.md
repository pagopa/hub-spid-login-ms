...
The `basic` scenario intentionally reuses the local `spid-testenv2` certificate pair generated by `../scripts/make-certs.sh`:
* `../certs/certificate.crt` is copied into `./scenarios/basic/res/spidsamlcheck-conf/idp.json` and `./scenarios/basic/res/spidsamlcheck-conf/idp_demo.json`
* `../certs/certificate.pem` is converted to the PKCS#1 payload required by `spid-saml-check` and copied into the same files as `privateKey`
