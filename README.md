...
<details>
  ...
  * Fill `METADATA_PUBLIC_CERT` with content of `certs/cert.pem` (generated by `make-certs.sh` for the `hub-spid-login-ms` metadata endpoint)
  * Fill `METADATA_PRIVATE_CERT` with content of `certs/key.pem` (generated by `make-certs.sh` for the `hub-spid-login-ms` metadata endpoint)
...
`scripts/make-certs.sh` generates two distinct certificate pairs:

- `certs/certificate.pem` and `certs/certificate.crt` for the local `spid-testenv2` HTTPS endpoint. The same pair is also propagated to the fake IdP configuration used by the `basic` e2e scenario.
- `certs/key.pem` and `certs/cert.pem` for the `hub-spid-login-ms` SP metadata exposed by this service and injected into `METADATA_PRIVATE_CERT` and `METADATA_PUBLIC_CERT`.

The `spid-testenv2` private key is written in PKCS#1 on purpose, because the `basic` scenario's `spid-saml-check` image expects the embedded `privateKey` value to be wrapped as `RSA PRIVATE KEY` when signing metadata.
